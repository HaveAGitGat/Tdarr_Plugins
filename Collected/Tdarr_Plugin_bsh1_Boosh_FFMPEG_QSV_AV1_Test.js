...
// This Plugin started as his NVENC/CPU plugin modified to work with QSV & with extra av1 logic.
// Rewritten to control encoder quality/speed & to allow AV1 files to be reprocessed to reduce file size
...
const details = () => ({
  id: 'Tdarr_Plugin_bsh1_Boosh_FFMPEG_QSV_AV1_Test',
  Stage: 'Pre-processing',
  Name: 'Boosh-Transcode using QSV GPU & FFMPEG to AV1 (TEST)',
  Type: 'Video',
  Operation: 'Transcode',
  Description: `==DETAILS== This is a QSV plugin. 8th+ gen INTEL QSV enabled CPUs are recommended. VAAPI is NOT used. 
    ...
    \n\n==LOGIC== Files will be transcoded into AV1 using Quick Sync Video (QSV) via Intel GPU using ffmpeg. 
    Settings are dependant on file bitrate working by the logic that H265 can support the same amount of data at half 
    the bitrate of H264. This plugin will skip files already in HEVC, AV1 & VP9 unless "reconvert_av1" is marked as 
    true. If it is then these will be reconverted again if they exceed the bitrate specified in "av1_max_bitrate".
  ...
  Version: '1.3',
  Tags: 'pre-processing,ffmpeg,video only,qsv,av1,configurable',
  Inputs: [
    {
      name: 'container',
      type: 'string',
    ...
    },
    {
      ...
      AV1 using main10 profile and with p010le pixel format.\n
    ...
    },
    {
      ...
      tooltip: `\\n
      ==DESCRIPTION==
      \\nSpecify the modifier for the target bitrate. The logic is that AV1 can obtain the same quality 
      at half the bitrate.
    ...
    },
    {
      ...
      https://ffmpeg.org/ffmpeg-codecs.html#AV1-Options
      \\n
      ==WARNING== \\n
    ...
    },
    {
      ...
      \\nThis is useful in certain situations, perhaps you have a file which is AV1 but has an extremely high
      ...
      \\nThis option has the potential to LOOP your encodes! You can encode a file to AV1 and still 
      be above your cutoff and it would be converted again & again if this is set to true (since it's now AV1). 
      So if you use this be sure to set "av1_max_bitrate" & "max_average_bitrate" to help prevent the plugin looping. 
      Also it is highly suggested that you have your "av1_max_bitrate" higher than "max_average_bitrate".
      ...
      and will be forced to fall back onto estimates. Please bare this in mind when using the AV1 reprocess option.
      \\n
      \\nExample:\\n
      true
      \\nExample:\\n
    ...
    },
    {
      ...
      tooltip: `\\n
      ==DESCRIPTION==
      \\nHas no effect unless "reconvert_av1" is set to true. This allows you to specify a maximum
      allowed average OVERALL bitrate for AV1 or similar files. Much like the "bitrate_cutoff" option, but
      specifically for AV1 files. It should be set HIGHER then your standard cutoff for safety.
      \\nAlso, it's highly suggested you use the min & max average bitrate options in combination with this. You
      will want those to control the encoded video bitrate, otherwise you may end up repeatedly reprocessing AV1 files.
      i.e your file might have a overall bitrate of 20000, if your av1 cutoff is 5000 then it's going to reconvert 
      multiple times before it'll fall below that cutoff. While AV1 reprocessing can be useful this is why it is NOT 
      recommended!
      \\n
      ==WARNING== \\n
      While the plugin will attempt to generate accurate video bitrate metadata, it can not always reliably do so 
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  if (Number.isNaN(videoBR) || videoBR <= 0) {
    ...
    if (Number.isNaN(currentBitrate) || currentBitrate <= 0) {
      response.infoLog += '==ERROR== Failed to get any bitrate data from this file!\n'
        ...
        + 'THIS ENCODE WILL ERROR FOR VISIBILITY';
    } else {
  ...
  // Get overall bitrate for use with AV1 reprocessing
  ...
  // Ensure that bitrate_cutoff is set if reconvert_av1 is true since we need some protection against a loop
  // Cancel the plugin
  if (inputs.reconvert_av1 === true && inputs.bitrate_cutoff <= 0 && inputs.av1_max_bitrate <= 0) {
    response.infoLog += `Reconvert AV1 is ${inputs.reconvert_av1}, however there is no bitrate cutoff or AV1 `
  ...
  if (inputs.bitrate_cutoff > 0) {
    ...
    if (currentBitrate > inputs.bitrate_cutoff && inputs.reconvert_av1 === false) {
  ...
  for (let i = 0; i < file.ffProbeData.streams.length; i += 1) {
    ...
    if (file.ffProbeData.streams[i].codec_type.toLowerCase() === 'video') {
      ...
      if (file.ffProbeData.streams[i].codec_name === 'mjpeg' || file.ffProbeData.streams[i].codec_name === 'png') {
        ...
        if ((file.ffProbeData.streams[i].codec_name === 'av1'
        ...
        if (inputs.reconvert_av1 === false && (file.ffProbeData.streams[i].codec_name === 'av1'
          ...
          if ((file.ffProbeData.streams[i].codec_name === 'av1' || file.ffProbeData.streams[i].codec_name === 'vp9'
            || file.ffProbeData.streams[i].codec_name === 'av1') && file.container === inputs.container) {
        ...
        } else if (inputs.reconvert_av1 === true && (file.ffProbeData.streams[i].codec_name === 'av1'
          || file.ffProbeData.streams[i].codec_name === 'vp9' || file.ffProbeData.streams[i].codec_name === 'av1')) {
          if (inputs.av1_max_bitrate > 0) {
            if (currentBitrate > inputs.av1_max_bitrate) {
              // If bitrate is higher then av1_max_bitrate then need to re-encode
              response.infoLog += `Reconvert_av1 is ${inputs.reconvert_av1} & the file is already HEVC, VP9 or AV1. `
                + `Using HEVC specific cutoff of ${inputs.av1_max_bitrate}kbps.\n`
                + '☒ The file is still above this new cutoff! Reconverting.\n';
            } else {
              // Otherwise we're now below the av1 cutoff and we can exit
              response.infoLog += `Reconvert_av1 is ${inputs.reconvert_av1} & the file is already HEVC, VP9 or AV1. `
                + `Using HEVC specific cutoff of ${inputs.av1_max_bitrate}kbps.\n`
            ...
            // If we're not using the av1 max bitrate then we need a safety net to try and ensure we don't keep
            ...
            if (inputs.bitrate_cutoff > 0) {
              ...
              response.infoLog += `Reconvert_av1 is ${inputs.reconvert_av1} & the file is already HEVC, VP9 or AV1. `
                + `Will use Overall file Bitrate for AV1 files as safety, bitrate is ${overallBitRate}kbps.\n`
  ...
  if (swDecode === true && main10 === true) {
    ...
    response.infoLog += '10 bit encode enabled. Setting 10 bit pixel format\n';
    ...
    if (inputs.extra_qsv_options.search('-vf scale_qsv') >= 0) {
      ...
      extraArguments += ' -vf scale_qsv=format=p010le';
    }
    response.infoLog += '10 bit encode enabled. Setting 10 bit pixel format\n';
  }
  ...
  // Account for different OS setup for QSV AV1 encode.
  switch (os.platform()) {
    case 'linux':
