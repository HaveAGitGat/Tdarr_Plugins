                    ...
                    case 0:
                        ...
                        return [4 /*yield*/, new Promise(function (resolve) {
                                try {
                                    ...
                                    // Match splits where a space is followed by `-` or `--`
                                    // to allow multiple args and args that have parameters
                                    // following a space such as for mkvpropedit
                                    // const spawnArgs = this.config.spawnArgs
                                    //  .map((row) => row.trim())
                                    //  .filter((row) => row !== '')
                                    //  .flatMap((row) => row.split(/ (?=-{1,2})/));
                                    var spawnArgs = _this.config.spawnArgs.map(function (row, index) {
                                        if (index === 0) {
                                            // Apply trimming, filtering, and splitting to the first element
                                            return row
                                                .trim()
                                                .split(/ (?=-{1,2})/); // Split the first element on spaces followed by - or --
                                        }
