...
const details = () => ({
  id: 'Tdarr_Plugin_FFMPEG_All_NVIDIA_GPUs',
  Stage: 'Pre-processing',
  Name: 'H265-Transcode Using Nvidia GPU & FFMPEG with load balancing',
  Type: 'Video',
  Operation: 'Transcode',
  Description: `Files not in H265 will be transcoded into H265 using Nvidia GPU with ffmpeg.
                  ...
                  This Plugin requires "nvidia-smi" to be installed and accessible from your node
  ...
  if (!isNaN(gpu_count)) {
    for (let gpui = 0; gpui < gpu_count; gpui++) {
      ...
      if (!isNaN(result_mem)) { // != "No devices were found") {
        ...
        response.infoLog += `Error in reading GPU ${gpui} Memory, Error: ${result_mem}\n`;
      }
    }
  }
