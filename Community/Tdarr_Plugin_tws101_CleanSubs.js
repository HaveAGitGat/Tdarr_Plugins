...
const details = () => ({
    id: 'Tdarr_Plugin_tws101_CleanSubs',
    Stage: 'Pre-processing',
    Name: 'tws101 - Clean subtitle streams',
    Type: 'Subtitle',
    Operation: 'Transcode',
    Description: 'This plugin keeps only specified language tracks & can tag tracks with an unknown language, and commentary as well as remove signs and song tracks. \n\n',
//    Created by Migz
//    Forked by tws101 
//    Release Version
    Version: '1.00',
  ...
  const plugin = (file, librarySettings, inputs, otherArguments) => {
    ...
    for (let i = 0; i < file.ffProbeData.streams.length; i++) {
      ...
      // Catch error here incase the title metadata is completely missing.
      try {
        // Check if inputs.sings_songs is set to true
        // AND if stream is subtitle
        // AND then checks for stream titles with the following "signs, songs".
        // Removing any streams that are applicable.
        if (
          inputs.signs_and_songs === true
          && file.ffProbeData.streams[i].codec_type.toLowerCase() === 'subtitle'
          && (file.ffProbeData.streams[i].tags.title
            .toLowerCase()
            .includes('signs')
          ...
          ffmpegCommandInsert += `-map -0:s:${subtitleIdx} `;
          response.infoLog += `☒Subtitle stream 0:s:${subtitleIdx} detected as being sign or song, removing. \n`;
          convert = true;
        }
      } catch (err) {
        // Error
