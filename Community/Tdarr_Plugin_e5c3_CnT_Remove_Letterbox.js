...
function plugin(file, librarySettings, inputs) {
    ...
    var min_crop = parseInt(file.meta.ImageHeight*.98); //if the crop value is larger than this the file won't be cropped
...
function create_crop_values(file) {
    ...
    //delete old crop value if it exists
    //if (fs.existsSync(`${cropfile}`)) {
    //    returns.log += `removing old cropvalues...\n`;
...
function crop_decider(file, crop_height) {
    ...
    if (file.meta.ImageWidth >= 1300) { //file will be encoded if the resolution is 1080p, or greater (it will be downscaled)
        ...
        if (crop_height < min_crop) {
            var crop_hdis = parseInt((file.meta.ImageHeight-crop_height)/2);
            if (crop_height >= 790) {
                returns.crop = `-filter:0 crop=1920:${crop_height}:0:${crop_hdis}`;
                returns.log += `☑ - crop is larger than 1%\n`;
            }
        } else {
            returns.log += `☒ - Crop is not necessary\n`;
        }
    } else if(file.meta.ImageWidth < 1300 && file.meta.ImageWidth >= 770) { 
        //crop only if it is a larger crop than 1%;
        if (crop_height < min_crop) {
            var crop_hdis = parseInt((file.meta.ImageHeight-crop_height)/2);
            if (crop_height >= 530) {
                returns.crop = `-filter:0 crop=1280:${crop_height}:0:${crop_hdis}`;
                returns.log += `☑ - crop is larger than 1%\n`;
            }
        } else {
            returns.log += `☒ - Crop is not necessary\n`;
        }
    } else if(file.meta.ImageWidth < 770) { //file won't be cropped at this resolution
