...
Inputs: [
      {
        ...
        How much does HEVC compress raw video?  I suggest something between 0.04-0.08.  Remember that GPU encoding is faster but not as 
        space efficient as CPU encoding, so resulting file sizes will be larger than if you used CPU encoding.\\n\\n
...
response.preset += `,-map 0:v -map 0:a -map 0:s? -map -:d? -c copy -c:v:0 hevc_nvenc -rc:v vbr_hq -preset ${inputs.ffmpegPreset} -profile:v main10 -rc-lookahead 32 -spatial_aq:v 1 -aq-strength:v 8 -max_muxing_queue_size 4096 `;
