// tdarrSkipTest
const details = () => ({
  id: 'Tdarr_Plugin_NG56_EatPrilosec_extract_subs_by_type_webvtt',
  Stage: 'Pre-processing',
  Name: 'Extract embedded WebVTT (webvtt) to .vtt and optionally remove them',
  Type: 'Video',
  Operation: 'Transcode',
  Description: 'This plugin extracts embedded WebVTT subs  \n\n ',
  Version: '1.00',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  for (let i = 0; i < subsArr.length; i += 1) {
    ...
    if (subStream.codec_name.toLowerCase().includes('webvtt')) {
      subsFile[subsFile.length - 1] = 'vtt';
      var thisformat = 'webvtt';
	  if (removecmd === 'none') {
