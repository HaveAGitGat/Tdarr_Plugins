// tdarrSkipTest
const details = () => ({
  id: 'Tdarr_Plugin_NG56_EatPrilosec_extract_subs_by_type_microdvd',
  Stage: 'Pre-processing',
  Name: 'Extract embedded MicroDVD (microdvd) to .sub and optionally remove them',
  Type: 'Video',
  Operation: 'Transcode',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  for (let i = 0; i < subsArr.length; i += 1) {
    ...
    if (subStream.codec_name.toLowerCase().includes('microdvd')) {
      subsFile[subsFile.length - 1] = 'sub';
      var thisformat = 'microdvd';
	  if (removecmd === 'none') {
