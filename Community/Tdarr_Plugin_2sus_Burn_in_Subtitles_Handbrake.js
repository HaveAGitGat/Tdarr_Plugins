const details = () => ({
  ...
  Inputs: [
    ...
    },
    {
      ...
      tooltip: `Specify the audio language subs should be burned in for. 
        \\If ONLY this language is present, burn in subs.`,
    },
    {
      name: 'sub_lang',
      type: 'string',
      ...
      tooltip: `Specify the language of subtitles to burn in. 
      If a comma separated list, burn in the first matching subtitle\n\n
            
      Example: "eng,und" burn in first matching eng subs. If none match eng, burn in first matching und subs`,
    },
  ],
});
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  const { audio_lang } = inputs;
  const { sub_lang } = inputs;

  const audio_streams = new Set(); // Find all audio streams
  const possible_sub_streams = []; // Find possible subtitle streams to burn in
  // let audio_streams = [];
  ...
  response.preset = `--preset "${inputs.handbrake_preset}" 
    --subtitle-burned --first-subtitle --subtitle-lang-list "${inputs.sub_lang}" ${inputs.extra_args}`;
